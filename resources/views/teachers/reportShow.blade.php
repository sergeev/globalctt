                ...
                <div class="card">
                    {{-- @foreach ($users as $user)
                        ...
                        <div class="card-header">
                            <p class="float-center">Мои отчеты по наполняемости</p>
                            {{--
                    @endforeach --}}
                    <a class="btn btn-primary float-right" href="{{ route('home') }}"> Назад</a>
                    <a class="btn btn-primary float-left" href="{{ route('teacher.reports.create') }}"> Добавление нового
                        отчета наполняемости </a>
                </div>
                <div class="card-body">
                    <table class="table">
                        <thead>
                                    ...
                                    @foreach ($users as $user)
                                        @if ($user->teacher_id == $teacher->teacher_full_id)
                                            {{-- @if ($report_list->teacher_full_name === $teacher->teacher_full_name)
                                                ...
                                                <td>
                                                    @if ($report_list->report_date_input !== $report_list->created_at)
